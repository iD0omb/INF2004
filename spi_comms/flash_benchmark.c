...
static void flash_write_addr(uint32_t addr) {
  uint8_t addr_bytes[3] = {(addr >> 16) & 0xFF, (addr >> 8) & 0xFF,
                           addr & 0xFF};
...
int flash_read_jedec_id(uint8_t *manufacturer, uint8_t *device_id_1,
                        uint8_t *device_id_2) {
  uint8_t id_data[3];
...
int flash_identify_chip(char *chip_name, size_t name_size) {
  ...
  if (manufacturer == 0xEF) { // Winbond
    if (device_id_1 == 0x40 && device_id_2 == 0x16) {
      ...
      snprintf(chip_name, name_size, "Winbond_Unknown_%02X%02X", device_id_1,
               device_id_2);
    }
    ...
    snprintf(chip_name, name_size, "Macronix_%02X%02X", device_id_1,
             device_id_2);
  } else if (manufacturer == 0x1F) { // Atmel/Microchip
    snprintf(chip_name, name_size, "Atmel_%02X%02X", device_id_1, device_id_2);
  } else {
    snprintf(chip_name, name_size, "Unknown_%02X_%02X%02X", manufacturer,
             device_id_1, device_id_2);
  }
...
void generate_test_pattern(uint8_t *buffer, uint32_t size,
...
uint64_t benchmark_flash_read(uint32_t address, uint32_t size,
                              const char *pattern) {
  if (!flash_initialized)
    return 0;
...
uint64_t benchmark_flash_program(uint32_t address, uint32_t size,
                                 const char *pattern) {
  if (!flash_initialized)
    return 0;
  ...
  while (remaining > 0) {
    uint32_t chunk_size =
        (remaining > FLASH_PAGE_SIZE) ? FLASH_PAGE_SIZE : remaining;
...
uint64_t benchmark_flash_erase(uint32_t address, uint32_t size) {
  ...
  while (remaining > 0) {
    ...
    remaining =
        (remaining > FLASH_SECTOR_SIZE) ? (remaining - FLASH_SECTOR_SIZE) : 0;
  }
